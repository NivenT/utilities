...
#define UTILS_OP_CHECKER(name, op) \
    ...
    template<typename T, typename U=T> \
    constexpr bool name##Exists_v = name##Exists<T, U>::value;
...
#define UTILS_UNARY_OP_CHECKER(name, op) \
    ...
    template<typename T> \
    constexpr bool name##Exists_v = name##Exists<T>::value;
...
#define UTILS_IMPL_WRAPPER_COMP(name, op) \
    ...
    template<typename U, typename std::enable_if_t<check:: name##Exists_v<T, U> && check::EqualsExists_v<T, U>>* = nullptr> \
    bool operator op##=(const U& rhs) const { \
...
#define UTILS_IMPL_WRAPPER_SELF_COMP(name, op) \
    ...
    template<typename T, typename TAG, typename std::enable_if_t<check:: name##Exists_v<T> && check::EqualsExists_v<T>>* = nullptr> \
    bool operator op##=(const Wrapper<T, TAG>& lhs, const Wrapper<T, TAG>& rhs) { \
...
namespace utils {
    ...
    public:
        ...
        template<typename std::enable_if_t<check::EqualsExists_v<T>>* = nullptr>
    ...
    template<typename T, typename TAG, typename std::enable_if_t<check::IncrExists_v<T>>* = nullptr>
    ...
    template<typename T, typename TAG, typename std::enable_if_t<check::DecrExists_v<T>>* = nullptr>
    Wrapper<T, TAG> operator--(Wrapper<T, TAG>& lhs, int) {
        auto ret = lhs;
        --lhs;
        return ret;
    }
    template<typename T, typename TAG, typename std::enable_if_t<check::LShiftExists_v<std::ostream, T>>* = nullptr>
