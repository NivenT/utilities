...
namespace utils {
    ...
    /// \todo Clean up code
    std::string Json::dump(size_t indent, size_t offset) const {
        switch(m_type) {
    ...
    bool Json::lex_number(std::string& str, JsonToken& ret) {
        static const unordered_set<char> FLOAT_CHARS{
            '.', 'e', 'E'
        };
    ...
    queue<Json::JsonToken> Json::tokenize(std::string curr) {
        static const unordered_set<char> JSON_SYNTAX{
            '{', '}', '[', ']', ':', ','
        };
    ...
    Json Json::parse_tokens(queue<JsonToken>& tokens) {
    ...
    Json Json::from_file(const std::string& path) {
        std::string contents;

        /// \todo Move functionality to common.h
        ifstream file(path.c_str(), ios::binary);
        if (file.fail()) return Json::null();
